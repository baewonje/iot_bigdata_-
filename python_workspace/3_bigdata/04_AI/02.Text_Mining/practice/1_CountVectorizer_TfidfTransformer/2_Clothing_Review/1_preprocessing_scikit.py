...
for line in range(len(reviews)):
    if not str(reviews['Review Text']): # 리뷰가 없을때 리스트에 저장하지 않음
        continue
...
word_count = vectorizer.fit_transform(user_reviews) # 리뷰에서 단어 횟수세기
...
#처리된 을 저장
with open('pre_review.pickle','wb') as file_handle:
